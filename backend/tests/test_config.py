...
def test_postgresql_database_url_selected_when_configured():
    """Test that a valid PostgreSQL DATABASE_URL is selected when configured."""
    with _without_testing_env():
        real_url = "postgresql://postgres:real_password@db.example.supabase.co:5432/postgres"
        settings = Settings(
            DATABASE_URL=real_url,
        ...
        assert settings.effective_db_url == real_url
        assert "postgresql" in settings.effective_db_url
        assert "sqlite" not in settings.effective_db_url
        assert settings.is_supabase_configured is True