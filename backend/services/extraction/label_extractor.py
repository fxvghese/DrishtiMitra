...
def normalize_ocr_text(text: str) -> str:
    ...
    # Preserve "Mfg By:" / "Mfg by" as manufacturer indicator; normalize only standalone "Mfg Date:" / "Mfg:" / "Mfg "
...
def extract_label_fields(raw_text: str, client_fields: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for i, line in enumerate(lines):
        ...
        if not extracted["manufacturer"]:
            # Match manufacturer keywords including "mfg by" (with optional colon) but NOT standalone "mfg:" or "mfg date" (those are date indicators)
            mfg_keywords = ["mfg by", "manufactured by", "manufacturer", "mfd by", "packer", "importer", "packed by"]
            # Also check for "mfg by:" pattern (normalized to "Mfg By:" by normalize_ocr_text)
            if any(kw in line_lower for kw in mfg_keywords) or re.search(r'\bmfg\s+by\b', line_lower):
                parts = [line]
        ...
        if not extracted["date"]:
            # Date keywords including "mfg" and "pkd" as date indicators (not manufacturer)
            date_keywords = ["pkd", "packed", "batch", "date", "mfg date", "pkd date", "mfg:", "mfg ", "pkd:"]
            exclude_keywords = ["mfg by", "manufactured by", "mfd by"]
            if any(kw in line_lower for kw in date_keywords) and not any(ex in line_lower for ex in exclude_keywords):
                extracted["date"] = line
            else:
        ...
        if not extracted["consumer_care"]:
            care_keywords = ["consumer care", "customer care", "helpline", "complaints", "email", "contact"]
            if any(kw in line_lower for kw in care_keywords):
                extracted["consumer_care"] = line
            else:
                # Also detect phone/email patterns anywhere in the line
