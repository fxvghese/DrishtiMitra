...
def get_ocr_service() -> BaseOCRService:
    ...
    Tries PaddleOCR first, falls back to RapidOCR if PaddleOCR is unavailable.
    """
    try:
        from paddleocr import PaddleOCR
        # Quick check if PaddleOCR can be imported
        _ = PaddleOCR
        logger.info("PaddleOCR available, using PaddleOCRService")
        return PaddleOCRService()
    except Exception as exc:
        logger.warning(f"PaddleOCR unavailable ({exc}), falling back to RapidOCR")
